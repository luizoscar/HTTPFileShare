...
def compress_directory(src_file):
    '''
    Compress a directory to a temporary file
    
    @param src_file: The source directory
    '''
    
    # Create the temp file
    filename = tempfile.gettempdir() + os.sep + os.path.basename(src_file) + '.tar.gz'
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hp:f:d:", ["Port=", "From=", "Downloads="])
    ...
    if must_delete_after:
        src_file = compress_directory(src_file)
